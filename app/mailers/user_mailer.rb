class UserMailer < ActionMailer::Base
  ...
  def winner_notification(title,bids_count,winner_id,winner_amount,creator)
    ...
    mail(:to => "#{@recipients}", :subject => "[TW Garage Sale] [SPAM] Auction results for \"#{@title}\"", :from => 'GarageSale@no-reply.thoughtworks.com')
  end
  ...
  def administrator_notification_expired(title,alladmins,creator)
    ...
    @recipients = "#{@creator}@thoughtworks.com"
    #@recipients = "twgs.twgs@gmail.com"
    mail(:to => "#{@recipients}", :subject => "[TW Garage Sale] [SPAM] Auction for \"#{@title}\" has expired", :from => 'GarageSale@no-reply.thoughtworks.com')
  end
  ...
  def seller_notification_quick_sale_almost_ends(title,creator)
    ...
    @recipients = "#{@creator}@thoughtworks.com"
    #@recipients = "twgs.twgs@gmail.com"
    mail(:to => "#{@recipients}", :subject => "[TW Garage Sale] [SPAM] Your sale of item \"#{@title}\" is about to end", :from => 'GarageSale@no-reply.thoughtworks.com')
  end
end
